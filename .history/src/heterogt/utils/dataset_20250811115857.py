...
class FineTuneEHRDataset(Dataset):
    ...
    def __getitem__(self, idx): # return tokenized input_ids, token_types, adm_index, age_sex, labels
        ...
        assert age_gender_index.size(1) == len(self.records[hadm_id]), \
            f"Age gender index length {age_gender_index.size(1)} does not match input IDs length {len(self.records[hadm_id])}"
...
def batcher(tokenizer, n_token_type=4, is_pretrain=False):
    def batcher_dev(batch):
        ...
        age_gender_index = torch.cat([F.pad(raw_age_gender_idx, (0, max_n_adms - raw_age_gender_idx.size(1)), "constant", tokenizer.convert_tokens_to_ids(["[PAD]"], voc_type="age_gender")[0]) for raw_age_gender_idx in raw_age_gender_index], dim=0)
        assert input_ids.shape == token_types.shape == adm_index.shape
        ...
        for row in range(age_gender_index.size(0)):
            count_age_gender = (age_gender_index[row] != 0).sum().item()
            expected = adm_num[row].item()
            assert count_age_gender == expected, \
                (f"Row {row}: counts mismatch. "
                 f"age_gender={count_age_gender}, expected adm_num={expected}")

        return input_ids, token_types, adm_index, age_gender_index, seq_len, adm_num, labels
