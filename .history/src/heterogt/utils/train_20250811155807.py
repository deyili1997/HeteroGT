...
def train_with_early_stopping(model, train_dataloader, val_dataloader, test_dataloader,
                              optimizer, loss_fn, device, early_stop_patience, task_type, task_index, epochs,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for step, batch in progress_bar:
            ...
            labels = batch[-1].float()  # [B] 或 [B, 1]
            # 任务索引（若模型需要Tensor）
            task_idx_tensor = torch.as_tensor(task_index, device=device, dtype=torch.long)

            try:
                with autocast(device_type=device_type):
                    prediction = model(*batch[:-1], task_idx_tensor)
                ...
                num_steps = step + 1
                current_loss = running_loss / num_steps
